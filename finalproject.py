...
db=mysql.connector.connect(user="root",passwd="ashish123",host="localhost") 
 
my_cursor=db.cursor() #getting the cursor object
my_cursor.execute("CREATE DATABASE IF NOT EXISTS Shop") #creating the database named shop

db=mysql.connector.connect(user="root",passwd="ashish123",host="localhost",database='Shop') 
my_cursor=db.cursor()
...
def prodtoTable():
    ...
    wn.destroy()
#Function to get details of the product to be added.. tkinter
def addProd(): 
    global prodName, prodPrice, date, Canvas1,  wn
    
    #Creating the window
    wn = tkinter.Tk() 
    wn.title("E-commerce Billing and Shoping by Ashish")
...
def removeProd():
   ...
   #Executing the query and showing the message box
    ...
    wn.destroy()
#Function to get product details from the user to be deleted.. tkinter
def delProd(): 
...
def newCust():    
    global wn,name1,name2,name3,name4,name5,date,custName
    #Creating a window
    wn = tkinter.Tk() 
    wn.title("E-commerce Billing and Shoping by Ashish")
    wn.configure(bg='lavender blush2')
    ...
    i=res[4]
    Label(labelFrame,text="%-50s%-50s"%(i[1],i[2]) ,fg='black').place(relx=0.07,rely=y)
    name5 = Entry(labelFrame)
    name5.place(relx=0.6,rely=y, relwidth=0.2)
    y += 0.1
...
headingFrame1.place(relx=0.2,rely=0.1,relwidth=0.6,relheight=0.16)
headingLabel = Label(headingFrame1, text="E-commerce Billing and Shoping \n by Ashish", fg='grey19', font=('Courier',15,'bold'))
headingLabel.place(relx=0,rely=0, relwidth=1, relheight=1)
